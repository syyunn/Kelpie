...
from kelpie.evaluation import MultiKelpieEvaluator
...
kelpie_dataset = MultiKelpieDataset(dataset=original_dataset, entity_id=original_entity_id)
kelpie_entity_id = kelpie_dataset.original_id_2_kelpie_id[original_entity_id]
...
perturbed_list, skipped_list = kelpie_perturbation.perturbate_samples(kelpie_entity_id, kelpie_train_samples)
print("Re-trainings to perform:" + str(len(perturbed_list)))
...
def run_kelpie(train_samples):
    ...
    kelpie_model = MultiKelpieComplEx(model=original_model, dataset=kelpie_dataset, init_size=1e-3) # type: KelpieComplEx
    ...
    print("\nKelpie model on original test fact to explain: <%s, %s, %s>" % original_triple)
    ...
    original_id_2_kelpie_id = kelpie_dataset.original_id_2_kelpie_id
    kelpie_sample_tuple_head = original_id_2_kelpie_id[head_id] if head_id in original_id_2_kelpie_id else head_id
    kelpie_sample_tuple_tail = original_id_2_kelpie_id[tail_id] if tail_id in original_id_2_kelpie_id else tail_id

    kelpie_sample_tuple = (kelpie_sample_tuple_head, relation_id, kelpie_sample_tuple_tail)
    ...
    print("\nKelpie model on kelpie version of the test fact to explain: <%s, %s, %s>" % kelpie_sample_tuple)
